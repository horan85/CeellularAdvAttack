...
clf = CellNN()
clf.cuda()
clf.load_state_dict(torch.load('CifarCellModel.pth'))
clf.eval()

Noise=torch.nn.Parameter(torch.rand((1,3,32,32),device='cuda'))
...
for AttackInd in range(100):
      ...
      if AttackLabel>=inlabel:
            AttackLabel+=1
      AttackLabel=torch.tensor(AttackLabel)
...
np.save('cifar_cell_attacks.npy',Attacks)
